<?php
	...
	require_once Path::reflect("src/reflect/Request.php");
	require_once Path::reflect("src/reflect/Endpoint.php");
	require_once Path::reflect("src/reflect/Method.php");
	require_once Path::reflect("src/reflect/Call.php");
	...
	class Router extends Auth {
		...
		// Request URLs starting with the configured internal prefix are reserved and should read from the internal endpoints located at /src/api/endpoints
		private function get_endpoint_file_path(): string {
			...
			if (substr($this->endpoint, 0, strlen($internal_prefix)) === $internal_prefix) {
				// Skip configured prefix name as the path is fixed in src
				$endpoint = str_replace($internal_prefix, "", $this->endpoint);

				return Path::reflect("src/api/endpoints{$endpoint}/{$this->method->value}.php");
			}
