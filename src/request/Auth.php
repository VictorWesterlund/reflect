<?php
	...
	class Auth extends Database {
		...
		private function get_user_groups(): array {
			...
			$resp = $this->for(UsersGroupsModel::TABLE)
				->where([
					UsersGroupsModel::REF_USER->value => self::$user_id
				])
				->select(UsersGroupsModel::REF_GROUP->value);

			// Extract group ID from database response
			return array_column($resp->fetch_all(MYSQLI_ASSOC), UsersGroupsModel::REF_GROUP->value);
		}
